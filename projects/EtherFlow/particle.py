...
class Particle:
    ...
    def move(self):
        ...
        # Out of bounds check.
        out_left = self.center_x < -1
        out_right = self.center_x > 1
        out_top = self.center_y > 1
        out_bottom = self.center_y < -1
        # self.velocity_x[out_left] *= -1
        # self.velocity_x[out_right] *= -1
        # self.velocity_y[out_top] *= -1
        # self.velocity_y[out_bottom] *= -1


        self.center_x[out_left] = 1
        self.center_x[out_right] = -1
        self.center_y[out_top] = -1
        self.center_y[out_bottom] = 1
